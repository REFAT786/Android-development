...
public class ContactRepository {
    ...
    public void update(ContactItem contactItem) {
        new UpdateContactAsyncTask(dao).execute(contactItem);
    }
    ...
    private static class UpdateContactAsyncTask extends AsyncTask<ContactItem, Void, Void> {
        private ContactDao dao;
        private UpdateContactAsyncTask(ContactDao dao) { //constructor as the class is static
            this.dao = dao;
        }
        @Override
        protected Void doInBackground(ContactItem... contactItems) {
            dao.Update(contactItems[0]);
            return null;
        }
    }
